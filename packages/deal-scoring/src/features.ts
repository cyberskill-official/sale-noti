import {
  ...
  type DealScoreMarket,
  type DealScoreObservation,
  type DealScoreRange,
  type DealScoreResult,
  type DealScoreWindow,
  type DealScoreWindowInput,
  type DealScoreLabel,
} from "./model";
...
function toDate(value: Date | string): Date {
  ...
  if (Number.isNaN(parsed.getTime())) throw new Error("INVALID_OBSERVED_AT");
  return parsed;
}
...
function normalizeObservations(observations: Array<DealScoreObservation>): Array<{ observedAt: Date; price: number; flashSale: boolean }> {
  return observations
    .map((observation) => ({
...
export function isDealScoreMarket(value: unknown): value is DealScoreMarket {
  return value === "VN" || value === "TH";
}

export function normalizeDealScoreMarket(value: unknown, fallback: DealScoreMarket = "VN"): DealScoreMarket {
  return isDealScoreMarket(value) ? value : fallback;
}

function discountThreshold(currentPrice: number, baselinePrice: number, currentDiscountPct: number): number {
...
function computeDiscountPersistenceHours(
  observations: Array<{ observedAt: Date; price: number; flashSale: boolean }>,
  currentPrice: number,
  ...
  if (!firstPersistent) return 0;
  return round(hoursBetween(firstPersistent.observedAt, latest.observedAt), 2);
}

export function extractDealScoreWindow(input: DealScoreWindowInput): DealScoreWindow {
  ...
  const categoryGapPct =
    categoryMedianPrice && categoryMedianPrice > 0 ? round(((categoryMedianPrice - currentPrice) / categoryMedianPrice) * 100, 2) : null;
...
function buildRecommendedPricePoint(window: DealScoreWindow): number | null {
...
function buildReasons(window: DealScoreWindow, unsupportedMarket: boolean): string[] {
  const reasons: string[] = [];

  if (unsupportedMarket) reasons.push("unsupported_market_fallback");
  ...
  if (window.currentPrice <= window.last30dMin * 1.02 && window.last30dMin > 0) reasons.push("at_30d_floor");
  ...
  if (window.flashSaleObserved) reasons.push("flash_sale_noise");
  if (window.reboundWithin24h) reasons.push("rebound_within_24h");
  if (window.observationCount < 3) reasons.push("thin_history");
  if (window.categoryMedianPrice == null) reasons.push("missing_category_median");
  if (window.daysSinceLastStrongDrop != null) reasons.push("known_recovery_window");

  return reasons.slice(0, 6);
}

function buildScore(window: DealScoreWindow, unsupportedMarket: boolean): DealScoreResult {
  ...
  if (window.flashSaleObserved) score -= 0.22;
  if (window.reboundWithin24h) score -= 0.18;
  if (window.categoryMedianPrice == null) score -= 0.03;
  if (window.observationCount < 3) score -= 0.05;
  ...
  let label: DealScoreLabel = "uncertain";
  if (!unsupported) {
    if (score >= 0.8) label = "real_deal";
    else if (score <= 0.35) label = "false_alarm";
  }
...
export function scoreUnsupportedDeal(window: DealScoreWindow): DealScoreResult {
