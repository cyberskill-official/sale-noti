...
export class CircuitBreaker {
  ...
  private onFailure(): void {
    ...
    const errRate = this.window.length > 0 ? this.window.filter((ok) => !ok).length / this.window.length : 0;
    if (
      this.consecFails >= this.opts.openAfterConsecFails ||
      (this.window.length === this.opts.windowCalls && errRate >= this.opts.openAfterErrorRate)
    ) {
      this.state = "open";
      this.openedAt = Date.now();
    }
  ...
  private recordWindow(ok: boolean): void {
    this.window.push(ok);
