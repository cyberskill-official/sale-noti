...
| FR-AFF-001    | Shopee signed client             | Unit/provider                 | Sign request; simulate retryable 429/5xx, breaker failures, token bucket rollover, clock skew retry, timeout mapping, safe telemetry; run live GraphQL smoke when credentials exist                    | SHA256 signature valid; retry/backoff capped at 3 internal retries; breaker opens/closes; no secret/raw payload logging                                                             | `sign.spec.ts`, `circuit-breaker.spec.ts`, `client.spec.ts`, `rate-limit-guard.spec.ts`; live provider handoff in `docs/qa/FR-AFF-001-live-handoff.md` |
| FR-AFF-002    | Deeplink attribution             | Unit/provider                 | Generate deeplink with user/watchlist/source/campaign; verify URL cross-check, cache hit, respect-publisher audit row/event, rate-limit, SET NX race behavior, and safe analytics                      | subIds present; cache/idempotency works; raw URL/userId absent from analytics; respect publisher avoids Shopee call; disclosure route controls click                                | `apps/api/src/affiliate/__tests__/deeplink.spec.ts`, `commission-reconcile.spec.ts`, disclosure tests                                                  |
| FR-AFF-003    | Product/shop offer resolver      | Unit/integration/provider     | Resolve live/dead product offers; write Mongo and Timescale rows; verify Redis shop-offer cache; verify flashSale threshold/explicit flag; verify Sentry phase tags and degraded Timescale behavior    | Product metadata and price observation persist; dead items are tombstoned; shop fallback is cached; errors are phase-tagged; Timescale outage does not break resolver response      | `offer-resolver.spec.ts`; live Shopee/DB smoke requires provider credentials                                                                           |
...
| FR-PRICE-001  | Timescale hypertable             | Unit/integration              | Split and apply separator-aware migrations; insert single/batch price rows; reject 1001-row batches; enforce raw-history 7d cap; query aggregate; run live migration against Timescale when URL exists | Hypertable migration is idempotent; batch and history APIs use one statement/aggregate paths; DB errors are redacted; live continuous aggregate validates with provider credentials | `timescale.client.spec.ts`, `timescale.spec.ts`, `migrate-lib.spec.mjs`; live DB path requires `TIMESCALE_DB_URL`                                      |
...
| FR-NOTIF-002  | Web push                         | Browser/provider              | Subscribe in Chrome, dispatch alert, click notification, revoke permission                                                                                                                             | Subscription saved; alert delivered; click tracked; expired endpoint cleaned                                                                                                        | `notify-push.spec.ts`, `push-routes.spec.ts`; browser/staging push smoke required                                                                      |
| FR-EXT-001    | Chrome MV3 extension             | Unit/manual Chrome            | Inspect manifest; load unpacked extension; open Shopee product page                                                                                                                                    | MV3, no `<all_urls>`, button appears after product URL, disclosure ack required                                                                                                     | `extension/tests/manifest.spec.ts`, `extension/tests/runtime.spec.ts`; manual Chrome load                                                              |
...
| FR-NOTIF-003  | Telegram bot                     | Provider/manual               | Generate link token; `/start`; dispatch alert; duplicate same idem key                                                                                                                                 | Chat linked; message sent with inline button; duplicate suppressed                                                                                                                  | `apps/api/src/notify/__tests__/telegram.spec.ts`; Telegram staging smoke required                                                                      |
| FR-GROW-001   | Referral anti-fraud              | Unit/API                      | Create qualified invites; test same IP/email-root/device abuse                                                                                                                                         | Three qualified invites unlock reward; suspicious cases flagged                                                                                                                     | `fraud-detect.spec.ts`, `referral-share.spec.ts`                                                                                                       |
| FR-GROW-002   | Share deal page                  | Web/API/browser               | Create share link; open `/deal/:slug`; click CTA                                                                                                                                                       | OG metadata, disclosure, click attribution, fallback if affiliate fails                                                                                                             | `referral-share.spec.ts`, `DealAffiliateActions.spec.tsx`; public social crawler smoke needed                                                          |
| FR-GROW-003   | Mega Sale Mode                   | Web/API/browser/manual social | Open sale page; inspect active/upcoming API; review manual social calendar                                                                                                                             | Event page renders; hot-tier schedule is bounded; manual posts prepared                                                                                                             | `megasale.spec.ts`, `adaptive-scheduler.spec.ts`, `priority-engine.spec.ts`; external auto-post manual gate                                            |
