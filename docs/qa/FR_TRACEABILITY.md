...
| FR                                      | Deliverables                                                                                                                                                                           | Verification                                                                                                                                               | Status                                                     |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| FR-AFF-001 Shopee Affiliate client      | Signed request helper, client, rate-limit guard, circuit breaker, typed responses, 3-attempt retry/backoff, safe telemetry                                                             | `sign.spec.ts`, `circuit-breaker.spec.ts`, `client.spec.ts`, `rate-limit-guard.spec.ts`; live Shopee smoke handoff in `docs/qa/FR-AFF-001-live-handoff.md` | Verified with mocks; live provider smoke credential-gated  |
| FR-AFF-002 generateShortLink            | Deeplink controller/service, attribution subIds, cache/idempotency, user rate-limit, respect-publisher audit row/event, disclosure interstitial hook, commission reconciliation worker | `deeplink.spec.ts`, `commission-reconcile.spec.ts`, disclosure tests; live Shopee short-link smoke needs credentials                                       | Verified with mocks; live provider smoke credential-gated  |
| FR-AFF-003 product/shop offer resolver  | Offer resolver, product upsert, Redis shop-offer cache, phase-tagged OBS, price-check processor, Timescale write path                                                                  | `offer-resolver.spec.ts`; DB/provider staging smoke; Timescale integration test when `TIMESCALE_DB_URL` is set                                             | Verified with mocks; live provider smoke credential-gated  |
...
| FR-PRICE-001 Timescale hypertable       | Separator-aware migrations, hardened hypertable SQL, typed Timescale client, batch insert, aggregate history, retention policies, redacted OBS telemetry                               | `timescale.client.spec.ts`, `timescale.spec.ts` (live DB skip without URL), `migrate-lib.spec.mjs`; run with Neon/Timescale URL before release             | Verified with mocks; live Timescale smoke credential-gated |
...
| FR-NOTIF-002 web push                   | Push processor, web subscribe/unsubscribe/click routes, service-worker-facing APIs                                                                                                     | `notify-push.spec.ts`, `push-routes.spec.ts`, targeted coverage; browser push smoke requires VAPID keys/profile                                           | Verified with mocks; live browser smoke credential-gated   |
...
| FR                           | Deliverables                                                                              | Verification                                                                        | Status             |
| ---------------------------- | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- | ------------------ |
| FR-BILL-001 freemium billing | Billing module/controller/service, plan constants, webhooks, grace-period cron            | `billing.service.spec.ts` with webhook/cron coverage; Stripe/VNPay/MoMo staging smoke required | Verified with mocks; live provider smoke credential-gated |
| FR-NOTIF-003 Telegram bot    | Telegram webhook controller, Telegram processor, web link-token route                     | `telegram.spec.ts`; provider staging smoke required                                  | Verified with mocks; live provider smoke credential-gated |
| FR-GROW-001 referral program | Referral controller/service, web `/r/:refCode`, fraud detection                           | `fraud-detect.spec.ts`                                                              | Verified           |
| FR-GROW-002 share deal       | Share controller/service, web deal page, share click API, OG metadata                     | `referral-share.spec.ts`, `DealAffiliateActions.spec.tsx`; short-link redirect follow-up noted | Verified for implemented scope |
| FR-GROW-003 Mega Sale Mode   | Mega Sale API/module, sale window config, web event page/banner, manual social schedule   | `megasale.spec.ts`, scheduler tests; external auto-post/manual social ops remain manual | Verified for implemented scope; manual provider gate |
